...
final class CadastroEspoleta extends JDialog {
	...
	private void salvar() {
		...
		if (opcao == JOptionPane.YES_OPTION) {
			espoleta.setDescricao(textFieldDescricao.getText().trim());
			espoleta.setQuantidade(Integer.parseInt(textFieldQuantidade.getText().trim()));
			Database.get().save(espoleta);
			dispose();
		}
	}
	...
	public CadastroEspoleta(Espoleta espoleta) {
		this();
		this.espoleta = espoleta;
		textFieldDescricao.setText(espoleta.getDescricao());
		textFieldQuantidade.setText(String.valueOf(espoleta.getQuantidade()));
	}

	/**
	 * Create the dialog.
	 */
	public CadastroEspoleta() {
		setModal(true);
