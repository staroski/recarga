package br.com.staroski.recarga.ui.chumbos;
...
public final class CadastroChumbo extends JDialog {
	...
	private void salvar() {
		...
		if (opcao == JOptionPane.YES_OPTION) {
			try {
				chumbo.setDescricao(textFieldDescricao.getText().trim());
				chumbo.setQuantidade(parseInt(textFieldQuantidade.getText()));
				Database.get().save(chumbo);
				dispose();
			} catch (Exception e) {
				showError(this, e);
			}
	...
	public CadastroChumbo(Chumbo chumbo) {
		this();
		this.chumbo = chumbo;
		textFieldDescricao.setText(chumbo.getDescricao());
		textFieldQuantidade.setText(String.valueOf(chumbo.getQuantidade()));
	}

	/**
	 * Create the dialog.
	 */
	public CadastroChumbo() {
		setModal(true);
