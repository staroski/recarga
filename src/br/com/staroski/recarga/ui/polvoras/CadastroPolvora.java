package br.com.staroski.recarga.ui.polvoras;
...
public final class CadastroPolvora extends JDialog {
	...
	private void salvar() {
		...
		if (opcao == JOptionPane.YES_OPTION) {
			try {
				polvora.setDescricao(textFieldDescricao.getText().trim());
				polvora.setQuantidade(parseInt(textFieldQuantidade.getText()));
				Database.get().save(polvora);
				dispose();
			} catch (Exception e) {
				showError(this, e);
			}
	...
	public CadastroPolvora(Polvora polvora) {
		this();
		this.polvora = polvora;
		textFieldDescricao.setText(polvora.getDescricao());
		textFieldQuantidade.setText(String.valueOf(polvora.getQuantidade()));
	}

	/**
	 * Create the dialog.
	 */
	public CadastroPolvora() {
		setModal(true);
